...
if ($_SESSION['RF']["verify"] != "RESPONSIVEfilemanager") {
    response(transs('forbidden').AddErrorLocation())->send();
    exit;
}
...
if (isset($_SESSION['RF']['language']) && file_exists('lang/' . basename($_SESSION['RF']['language']) . '.php')) {
    if (array_key_exists($_SESSION['RF']['language'], $languages)) {
        ...
        response(transs('Lang_Not_Found').AddErrorLocation())->send();
        exit;
    }
} else {
    response(transs('Lang_Not_Found').AddErrorLocation())->send();
...
if (isset($_GET['file']) && !checkRelativePath($_GET['file'])) {
    response(transs('wrong path').AddErrorLocation())->send();
    exit;
}
...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        case 'new_file_form':
            echo transs('Filename') . ': <input type="text" id="create_text_file_name" style="height:30px"> <select id="create_text_file_extension" style="margin:0;width:100px;">';
        ...
        case 'view':
            if (isset($_GET['type'])) {
                ...
                response(transs('view type number missing').AddErrorLocation())->send();
                exit;
            }
            break;
		...
		case 'save_img':
            ...
            if (preg_match('/^data:image\/(\w+);base64,/', $image_data, $type)) {
                ...
                if ($image_data === false) {
                    response(transs('TUI_Decode_Failed').AddErrorLocation())->send();
                exit;
                }
            } else {
                response(transs('').AddErrorLocation())->send();
                exit;
            }
            ...
            if (!checkresultingsize(strlen($image_data))) {
                response(sprintf(transs('max_size_reached'), $config['MaxSizeTotal']).AddErrorLocation())->send();
                exit;
            }
            if ($ftp) {
                $temp = tempnam('/tmp', 'RF');
        ...
        case 'extract':
            if (!$config['extract_files']) {
                response(transs('wrong action').AddErrorLocation())->send();
            }
            if ($ftp) {
            ...
            switch ($info['extension']) {
                case "zip":
                    ...
                    if ($zip->open($path) === true) {
                        ...
                        response(transs('Zip_No_Extract').AddErrorLocation())->send();
                        exit;
                    }
                ...
                default:
                    response(transs('Zip_Invalid').AddErrorLocation())->send();
                    exit;
            }
        ...
        case 'copy_cut':
            if ($_POST['sub_action'] != 'copy' && $_POST['sub_action'] != 'cut') {
                response(transs('wrong sub-action').AddErrorLocation())->send();
                exit;
            }

            if (trim($_POST['path']) == '') {
                response(transs('no path').AddErrorLocation())->send();
                exit;
            }

            $msg_sub_action = ($_POST['sub_action'] == 'copy' ? transs('Copy') : transs('Cut'));
            $path = $config['current_path'] . $_POST['path'];

            if (is_dir($path)) {
                // can't copy/cut dirs
                if ($config['copy_cut_dirs'] === false) {
                    response(sprintf(transs('Copy_Cut_Not_Allowed'), $msg_sub_action, transs('Folders')).AddErrorLocation())->send();
                    exit;
                }
                ...
                if ($config['copy_cut_max_size'] !== false && is_int($config['copy_cut_max_size'])) {
                    if (($config['copy_cut_max_size'] * 1024 * 1024) < $sizeFolderToCopy) {
                        response(sprintf(transs('Copy_Cut_Size_Limit'), $msg_sub_action, $config['copy_cut_max_size']).AddErrorLocation())->send();
                        exit;
                    }
                }
                ...
                if ($config['copy_cut_max_count'] !== false && is_int($config['copy_cut_max_count'])) {
                    if ($config['copy_cut_max_count'] < $fileNum) {
                        response(sprintf(transs('Copy_Cut_Count_Limit'), $msg_sub_action, $config['copy_cut_max_count']).AddErrorLocation())->send();
                        exit;
                    }
                }
                ...
                if ($config['copy_cut_files'] === false) {
                    response(sprintf(transs('Copy_Cut_Not_Allowed'), $msg_sub_action, transs('Files')).AddErrorLocation())->send();
                    exit;
                }
            }
        ...
        case 'chmod':
            if ($ftp) {
                ...
                if (
                    ...
                    response(sprintf(transs('File_Permission_Not_Allowed'), (is_dir($path) ? transs('Folders') : transs('Files')), 403).AddErrorLocation())->send();
                    exit;
                }
            ...
            if ((!$ftp && is_dir($path))) {
                $ret .= '<div class="hero-unit" style="padding:10px;">'.transs('File_Permission_Recursive').'<br/><br/>
                        <ul class="unstyled">
                            <li><label class="radio"><input value="none" name="apply_recursive" type="radio" checked> '.transs('No').'</label></li>
                            <li><label class="radio"><input value="files" name="apply_recursive" type="radio"> '.transs('Files').'</label></li>
                            <li><label class="radio"><input value="folders" name="apply_recursive" type="radio"> '.transs('Folders').'</label></li>
                            <li><label class="radio"><input value="both" name="apply_recursive" type="radio"> '.transs('Files').' & '.transs('Folders').'</label></li>
                        </ul>
                        </div>';
            }
        ...
        case 'cad_preview':
            if ($ftp) {
                ...
                if (! file_exists($selected_file)) {
                    response(transs('File_Not_Found').AddErrorLocation())->send();
                    exit;
                }
            }
            if ($ftp) {
        ...
        case 'get_file': // preview or edit
            ...
            if (! in_array($info['extension'], $allowed_file_exts)
                ...
                response(sprintf(transs('File_Open_Edit_Not_Allowed'), ($sub_action == 'preview' ? strtolower(transs('Open')) : strtolower(transs('Edit')))).AddErrorLocation())->send();
                exit;
            }
            ...
            break;
        default:
            response(transs('no action passed').AddErrorLocation())->send();
            exit;
    }
} else {
    response(transs('no action passed').AddErrorLocation())->send();

...
// $storeFolder = __DIR__ . '/' . $storeFolder; // correct for when IE is in Compatibility mode
// $storeFolderThumb = __DIR__ . '/' . $storeFolderThumb;
...
if (!empty($_FILES)) {
    ...
    if (in_array(fix_strtolower($extension), $ext)) {
        ...
        $targetFile = __DIR__ . '/' . $targetPath . $_FILES['file']['name'];
        $targetFileThumb = __DIR__ . '/' . $targetPathThumb . $_FILES['file']['name'];

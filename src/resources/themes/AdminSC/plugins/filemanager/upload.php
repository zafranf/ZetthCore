...
try {
    ...
    if ($_SESSION['RF']["verify"] != "RESPONSIVEfilemanager") {
        response(transs('forbidden') . AddErrorLocation(), 403)->send();
        exit;
    }
    ...
    $messages = null;
    if (transs("Upload_error_messages") !== "Upload_error_messages") {
        $messages = transs("Upload_error_messages");
    }
    ...
    if (!checkresultingsize($_FILES['files']['size'][0])) {
        ...
        $upload_handler->response['files'][0]->error = sprintf(transs('max_size_reached'), $config['MaxSizeTotal']) . AddErrorLocation();

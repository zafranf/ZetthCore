...
if (!checkRelativePath($_POST['path']) || strpos($_POST['path'], '/') === 0) {
    response(transs('wrong path') . AddErrorLocation(), 400)->send();
    exit;
}
...
if (!check_extension($info['extension'], $config)) {
    response(transs('wrong extension') . AddErrorLocation(), 400)->send();
    exit;
}
...
if ($ftp) {
    ...
    if (!file_exists($path . $name)) {
        response(transs('File_Not_Found') . AddErrorLocation(), 404)->send();
        exit;
    }

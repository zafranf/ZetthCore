  ...
  <script>
    $(document).ready(function() {
      let options = {
        "processing": true,
        "serverSide": true,
        "ajax": SITE_URL + "{{ adminPath() }}/content/gallery/videos/data",
        "pageLength": 20,
        "lengthMenu": [
          [10, 20, 50, 100, -1], 
          [10, 20, 50, 100, "All"]
        ...
        "columnDefs": [{
          ...
          "render": function (data, type, row, meta) {
            var actions = '';
            var url = SITE_URL + "{{ adminPath() }}/content/gallery/videos/" + data;
            var del = "_delete('" + url + "')";
            {!! _get_access_buttons() !!}
        ...
        options.columnDefs = [
          {
            ...
            "render": function (data, type, row, meta) {
              let actions = '';
              let url = SITE_URL + "{{ adminPath() }}/content/gallery/videos/" + data;
              let del = "_delete('" + url + "')";
              {!! _get_access_buttons() !!}
              $('[data-toggle="tooltip"]').tooltip();

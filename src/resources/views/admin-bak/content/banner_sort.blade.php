...
function sortMenu($data, $level = 0) {
  ...
  foreach ($data as $banner) {
    echo '<li data-id="' . $banner->id . '" data-name="' . $banner->title . '">' . ($banner->title != '' ? $banner->title : $banner->order);
    echo '</li>';
  }
  echo '</ol>';
}
...
@section('menu-sort')
  @if (\Auth::user()->can('update-menus'))
    <a href="{{ url('/content/banners/sort') }}" class="btn btn-info" data-toggle="tooltip" title="Urutkan"><i class="fa fa-sort"></i></a>
  @endif
@endsection
  ...
  <div class="card-body">
    ...
    <div class="form-group row">
      <div class="col-sm-12" style="padding-left: 40px;">
        <form action="{{ url('/content/banners/sort') }}" method="post">
          @csrf
          {{ method_field('put') }}
